...
print("=" * 70)
print("STUDYNOVA FINAL VERIFICATION REPORT")
print("=" * 70)
...
# Check if we can get subjects for different combinations
test_cases = [
    (1, 4, 1, "CSE - 4th Semester"),
    (1, 4, 2, "ISE - 4th Semester"),
    (1, 5, 3, "AIML - 5th Semester"),
    (1, 4, 10, "CSE_CS - 4th Semester"),
]

for sem_num, branch_id, branch_name in [
    (4, 1, "CSE"),
    (4, 2, "ISE"),
    (5, 3, "AIML"),
    (4, 10, "CSE_CS")
    ...
    cursor.execute('''
        ...
        WHERE s.scheme_id = ? AND s.semester_number = ?
    ''', (scheme_id, sem_num))
    sem = cursor.fetchone()
    if sem:
        sem_id = sem['id']
        ...
        print(f"   {status} Semester {sem_num} → {branch_name:10}: {subj_count:2} subjects")
...
print(f"✓ All required semesters present (3-7)")
print(f"✓ Total subjects: {total_subjects}")
print(f"✓ Fallback subjects removed")
